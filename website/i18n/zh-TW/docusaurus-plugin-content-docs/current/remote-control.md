...
被共享的 session 是一個終端機、兩個觀看者，而一個終端機只有一種尺寸。那個尺寸屬於
你啟動時所在的終端機：它就在那裡，正用那個尺寸畫著畫面，而一個把 agent 改成自己
視窗大小的瀏覽器，會讓那個終端機用它其實已經沒有的寬度去畫一個全螢幕 TUI ——
邊框折行、線條重複、游標跑掉。

所以頁面不會去改它的尺寸。頁面改成照 agent 真正的格線去畫，在空間允許的範圍內
盡量放大、置中，比例對不上的地方留黑 —— 就像播放器遇到跟螢幕比例不同的影片那樣。
你把終端機拉大縮小，頁面幾秒內就跟上。

`--tmux` 是給「頁面才是你真正要用的那一邊」的情況。它讓 agent 跑在 tmux 裡，而
tmux 正是為此而生的工具：一個程式，多個各自附著的 client，每個都有自己的尺寸。
...
而這一次，決定 agent 世界有多寬的是頁面。

### 有什麼不同

| | 不加 `--tmux` | 加了 `--tmux` |
| --- | --- | --- |
| 尺寸 | 你的終端機決定，頁面縮放後放進畫面 | 頁面決定，你的終端機顯示放得下的部分 |
| 卸離 | `ctrl-\` 然後 `d` | `ctrl-b` 然後 `d` |
| 捲動紀錄 | 頁面的 | 頁面的，外加你終端機裡 tmux 自己的複製模式 |
| 你本機看到的畫面 | 經過 hub 鏡像，金鑰被遮蔽 | 直接的 tmux client，原始輸出 |

第一列也值得看兩次。加了 `--tmux` 之後，比瀏覽器視窗更窄或更矮的終端機會看到
agent 畫面的**左上角**，而且不會有任何說明 —— 用 `ctrl-b :refresh-client
-L/-R/-U/-D` 平移，或 `-c` 讓它跟著游標。完全沒有瀏覽器接上時，尺寸就停在 session
啟動時的樣子，因為你的終端機對它沒有投票權：這就是 `--tmux` 的取捨，而
`alc share` 和 `alc attach` 都會印一行說明這件事。
...
那一列同時也是尺寸為什麼這樣安排的原因。tmux 服務一個尺寸和 window 不同的 client
時，會把整個 pane 一列一列重新送進去，而 alc 的金鑰遮蔽是比對連續位元組的 —— 所以
被這樣對待的 client 絕不能是鏡像。由頁面驅動時，鏡像永遠剛好就是 window 的尺寸，
被重送的是你自己的終端機，而那本來就不是 alc 在遮蔽的對象。
...
alc --share --tmux <agent>   # ……用 tmux：兩種尺寸，而 agent 的那個由頁面決定
