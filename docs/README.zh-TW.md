...
tmux 正是為此而生的工具：一個程式，多個各自附著、各有尺寸的 client：

```sh
alc --share --tmux --codex claude    # 或 -t
```

你的終端機和 hub 各自以獨立的 tmux client 附著上去，所以誰都不必遷就誰。有三件事
要知道：

- **尺寸由頁面決定，你的終端機顯示放得下的部分。** 瀏覽器視窗驅動 agent；比它更窄
  或更矮的終端機會看到畫面的左上角 —— 用 `ctrl-b :refresh-client -L/-R/-U/-D` 平移，
  或 `-c` 讓它跟著游標。這和不加 `--tmux` 的共享 session 剛好相反，而這正是取捨：
  會去要 `--tmux` 的人，接下來就是要去用那個頁面。
- **沒有瀏覽器接上時，尺寸就停在啟動時的樣子。** 你的終端機沒有投票權，所以在頁面
  連上之前，用另一個尺寸的終端機 `alc attach` 不會改變任何東西。
